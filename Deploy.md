...
cd purepicks/purepicks-backend && sh build.sh
...
启动后，可以通过命令tail -f purepicks-backend_startup.log观察日志情况。

ps 1: 可以动态适合自己key,编辑 purepicks/purepicks-backend/src/main/resources/application.yml,其中配置是可以添加多个模型，然后在不同模块下可以指定，比如在react模式下，我指定了claude-3-7-sonnet-v1，建议修改为适合自己的模型名字。
