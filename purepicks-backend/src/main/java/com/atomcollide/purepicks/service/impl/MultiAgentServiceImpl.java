...
import com.atomcollide.purepicks.agent.enums.AutoBotsResultStatus;
import com.atomcollide.purepicks.agent.enums.ResponseTypeEnum;
import com.atomcollide.purepicks.config.purepicksConfig;
import com.atomcollide.purepicks.handler.AgentResponseHandler;
...
public class MultiAgentServiceImpl implements IMultiAgentService {
    ...
    public void handleMultiAgentRequest(AgentRequest autoReq,SseEmitter sseEmitter) {
        ...
        OkHttpClient client = new OkHttpClient.Builder()
                ...
                .readTimeout(purepicksConfig.getSseClientReadTimeout(), TimeUnit.SECONDS)    // 设置读取超时时间为 60 秒
                .writeTimeout(1800, TimeUnit.SECONDS)   // 设置写入超时时间为 60 秒
                .callTimeout(purepicksConfig.getSseClientConnectTimeout(), TimeUnit.SECONDS)    // 设置调用超时时间为 60 秒
                .build();
    ...
    private AgentRequest buildAgentRequest(GptQueryReq req) {
        ...
        request.setSopPrompt(request.getAgentType() == 3 ? purepicksConfig.getpurepicksSopPrompt(): "");
        request.setBasePrompt(request.getAgentType() == 5 ? purepicksConfig.getpurepicksBasePrompt() : "");
