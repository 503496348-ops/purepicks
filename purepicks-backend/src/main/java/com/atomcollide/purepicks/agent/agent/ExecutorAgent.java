...
import com.atomcollide.purepicks.agent.dto.Message;
import com.atomcollide.purepicks.agent.dto.tool.ToolCall;
import com.atomcollide.purepicks.agent.dto.tool.ToolChoice;
import com.atomcollide.purepicks.agent.enums.AgentState;
import com.atomcollide.purepicks.agent.enums.RoleType;
import com.atomcollide.purepicks.agent.llm.LLM;
import com.atomcollide.purepicks.agent.prompt.ToolCallPrompt;
import com.atomcollide.purepicks.agent.tool.BaseTool;
import com.atomcollide.purepicks.agent.util.FileUtil;
...
public class ExecutorAgent extends ReActAgent {
    ...
    public ExecutorAgent(AgentContext context) {
        ...
        purepicksConfig purepicksConfig = applicationContext.getBean(purepicksConfig.class);
        ...
        setSystemPrompt(purepicksConfig.getExecutorSystemPromptMap().getOrDefault(promptKey, ToolCallPrompt.SYSTEM_PROMPT)
                ...
                .replace("{{executorSopPrompt}}", purepicksConfig.getExecutorSopPromptMap().getOrDefault(sopPromptKey, "")));
        setNextStepPrompt(purepicksConfig.getExecutorNextStepPromptMap().getOrDefault(nextPromptKey, ToolCallPrompt.NEXT_STEP_PROMPT)
                .replace("{{tools}}", toolPrompt.toString())
                .replace("{{query}}", context.getQuery())
                .replace("{{date}}", context.getDateInfo())
                .replace("{{sopPrompt}}", context.getSopPrompt())
        ...
        setMaxSteps(purepicksConfig.getPlannerMaxSteps());
        setLlm(new LLM(purepicksConfig.getExecutorModelName(), ""));

        setContext(context);
        setMaxObserve(Integer.parseInt(purepicksConfig.getMaxObserve()));

        // 初始化工具集合
        availableTools = context.getToolCollection();
        setDigitalEmployeePrompt(purepicksConfig.getDigitalEmployeePrompt());
    ...
    public String act() {
        if (toolCalls.isEmpty()) {
            ...
            setState(AgentState.FINISHED);
            // 删除工具结果
            if ("1".equals(purepicksConfig.getClearToolMessage())) {
                getMemory().clearToolContext();
            }
            // 返回固定话术
            if (!purepicksConfig.getTaskCompleteDesc().isEmpty()) {
                return purepicksConfig.getTaskCompleteDesc();
            }
    ...
    public String run(String request) {
        ...
        purepicksConfig purepicksConfig = SpringContextHolder.getApplicationContext().getBean(purepicksConfig.class);
        request = purepicksConfig.getTaskPrePrompt() + request;
        // 更新当前task
