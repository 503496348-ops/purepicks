...
import com.atomcollide.purepicks.agent.prompt.PlanningPrompt;
import com.atomcollide.purepicks.agent.tool.BaseTool;
import com.atomcollide.purepicks.agent.tool.common.PlanningTool;
...
public class PlanningAgent extends ReActAgent {
    ...
    public PlanningAgent(AgentContext context) {
        ...
        setSystemPrompt(purepicksConfig.getPlannerSystemPromptMap().getOrDefault(promptKey, PlanningPrompt.SYSTEM_PROMPT)
        ...
        setNextStepPrompt(purepicksConfig.getPlannerNextStepPromptMap().getOrDefault(nextPromptKey, PlanningPrompt.NEXT_STEP_PROMPT)
                .replace("{{tools}}", toolPrompt.toString())
                .replace("{{query}}", context.getQuery())
                .replace("{{date}}", context.getDateInfo())
                .replace("{{sopPrompt}}", context.getSopPrompt()));
        ...
        setLlm(new LLM(purepicksConfig.getPlannerModelName(), ""));

        setContext(context);
        setIsColseUpdate("1".equals(purepicksConfig.getPlanningCloseUpdate()));
    ...
    public String run(String request) {
        if (Objects.isNull(planningTool.getPlan())) {
            ...
            request = purepicksConfig.getPlanPrePrompt() + request;
        }
