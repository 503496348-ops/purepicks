package com.atomcollide.purepicks.agent.llm;
...
import com.atomcollide.purepicks.agent.dto.tool.McpToolInfo;
...
public class LLM {
    ...
    public static List<Map<String, Object>> formatMessages(List<Message> messages, boolean isClaude) {
        ...
        for (Message message : messages) {
            ...
            if (message.getBase64Image() != null && !message.getBase64Image().isEmpty()) {
                ...
                String content = StringUtil.textDesensitization(message.getContent(), purepicksConfig.getSensitivePatterns());
                if (isClaude) {
                    // Claude格式的工具调用结果处理
    ...
    public CompletableFuture<ToolCallResponse> askTool(
        ...
        try {
            ...
            if ("struct_parse".equals(functionCallType)) {
                purepicksConfig purepicksConfig = SpringContextHolder.getApplicationContext().getBean(purepicksConfig.class);
                stringBuilder.append(purepicksConfig.getStructParseToolSystemPrompt());
                // base tool
    ...
    public CompletableFuture<ToolCallResponse> callOpenAIFunctionCallStream(AgentContext context, Map<String, Object> params) {
        ...
        try {
            ...
            purepicksConfig purepicksConfig = SpringContextHolder.getApplicationContext().getBean(purepicksConfig.class);
            String[] interval = purepicksConfig.getMessageInterval().getOrDefault("llm", "1,3").split(",");
