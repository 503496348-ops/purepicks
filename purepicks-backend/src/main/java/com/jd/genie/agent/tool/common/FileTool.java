...
import com.atomcollide.purepicks.agent.dto.FileResponse;
...
public class FileTool implements BaseTool {
    ...
    public String getDescription() {
        ...
        return purepicksConfig.getFileToolDesc().isEmpty() ? desc : purepicksConfig.getFileToolDesc();
    }
    ...
    public Map<String, Object> toParams() {
        ...
        purepicksConfig purepicksConfig = SpringContextHolder.getApplicationContext().getBean(purepicksConfig.class);
        if (!purepicksConfig.getFileToolDesc().isEmpty()) {
            return purepicksConfig.getFileToolPamras();
        }
    ...
    public String uploadFile(FileRequest fileRequest, Boolean isNoticeFe, Boolean isInternalFile) {
        ...
        String url = purepicksConfig.getCodeInterpreterUrl() + "/v1/file_tool/upload_file";
    ...
    public String getFile(FileRequest fileRequest, Boolean noticeFe) {
        ...
        purepicksConfig purepicksConfig = applicationContext.getBean(purepicksConfig.class);
        MediaType mediaType = MediaType.get("application/json; charset=utf-8");
        String url = purepicksConfig.getCodeInterpreterUrl() + "/v1/file_tool/get_file";
        // 构建请求体
        ...
        try {
            ...
            if (Objects.nonNull(fileContent)) {
                if (fileContent.length() > purepicksConfig.getFileToolContentTruncateLen()) {
                    fileContent = fileContent.substring(0, purepicksConfig.getFileToolContentTruncateLen());
                }
