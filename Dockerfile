...
WORKDIR /app
COPY purepicks-backend/pom.xml .
COPY purepicks-backend/src ./src
COPY purepicks-backend/build.sh purepicks-backend/start.sh ./
...
COPY purepicks-backend/start.sh /app/backend/
...
# 复制 purepicks-client
WORKDIR /app/client
COPY purepicks-client/pyproject.toml purepicks-client/uv.lock ./
COPY purepicks-client/app ./app
COPY purepicks-client/main.py purepicks-client/server.py purepicks-client/start.sh ./
...
# 复制 purepicks-tool
WORKDIR /app/tool
COPY purepicks-tool/pyproject.toml purepicks-tool/uv.lock ./
COPY purepicks-tool/purepicks_tool ./purepicks_tool
COPY purepicks-tool/server.py purepicks-tool/start.sh purepicks-tool/.env_template ./
...
RUN chmod +x start.sh && \
    ...
    . .venv/bin/activate && \
    export UV_DEFAULT_INDEX="https://pypi.tuna.tsinghua.edu.cn/simple" && uv sync && \
    mkdir -p /data/purepicks-tool && \
    cp .env_template .env && \
    python -m purepicks_tool.db.db_engine

# 设置数据卷
VOLUME ["/data/purepicks-tool"]

# 复制统一启动脚本
WORKDIR /app
COPY start_purepicks.sh .
RUN chmod +x start_purepicks.sh
