...
# 配置文件路径
CONFIG_FILE="purepicks-backend/src/main/resources/application.yml"
ENV_TEMPLATE="purepicks-tool/.env_template"
ENV_FILE="purepicks-tool/.env"
...
init_setup() {
    ...
    if python -m purepicks_tool.db.db_engine; then
...
show_service_info() {
    ...
    echo -e "  - 查看日志: tail -f purepicks-backend/purepicks-backend_startup.log"
    echo -e "  - 重新启动: ./start_purepicks_one_click.sh"
    echo "=================================="
}
...
cleanup() {
    ...
    # 清理临时文件
    rm -f purepicks-backend/src/main/resources/application.yml.bak
    rm -f purepicks-tool/.env.bak
...
# 主函数
main() {
    echo -e "${BLUE}🚀 PurePicks PurePicks 一键启动脚本${NC}"
    echo "=================================="
